...
function LightModeIcon({label, className}: IconProps) {
    return (
...
function DarkModeIcon({label, className}: IconProps) {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
             className={className} aria-label={label}>
            <path
...
function SystemModeIcon({label, className}: IconProps) {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
             className={className} aria-label={label}>
            <path
...
type ColorModeTransition = {
    light: string;
    dark: string;
    system: string;
}

function getColorModeIron(mode: ColorMode, transition: ColorModeTransition) {
    if (mode === "light") {
        return <LightModeIcon label={transition.light}/>;
    } else if (mode === "dark") {
        return <DarkModeIcon label={transition.dark}/>;
    } else {
        return <SystemModeIcon label={transition.system}/>;
    }
}

function HeaderColorToggle() {
    const {colorMode, systemColorMode, toggleColorMode} = useColorMode();
    const t = useTranslations("header.HeaderColorToggle");
    ...
    return (
        <button onClick={listener} aria-label={t("switch")} title={t("switch")} type="button"
                className={clsx("p-3 shrink-0 flex cursor-pointer items-center hover:bg-bg-hover hover:text-link-hover fill-current")}>
            {getColorModeIron(colorMode, {light: t("light"), dark: t("dark"), system: t("system")})}
        </button>
    );
}
